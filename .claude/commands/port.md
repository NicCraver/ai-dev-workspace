...
用 Superpowers writing-plans 产出移植计划，追加写入 `context/features/<feature>/plan.md` 的「<目标端> 移植」小节。要求：
...
plan 经用户确认后按 Superpowers 执行流程实施。完成后自动进入 wrapup 技能（更新 status.md 矩阵中该端的格子、提交 context）。
