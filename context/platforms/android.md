...
- **正式包偏大 4.9MB = NDK 没装对，native 库没 strip**：AGP 4.2.2 的 `stripPublishReleaseDebugSymbols` 只认老式 NDK 路径 `toolchains/<triple>-4.9/prebuilt/darwin-x86_64/bin/<triple>-strip`；只装了 NDK 23+（如 28.x，仅剩 `toolchains/llvm/`）时找不到 strip 工具，Gradle 打一行 `Unable to strip the following libraries, packaging them as they are` 就原样打包，`libc++_shared.so`/`libvideoplayer.so`/`libqysdk.so` 带着调试符号进包，APK 从 **81.5MB 涨到 86.4MB**（dex/res/assets 完全一致，差量全在 `lib/`）。修：`sdkmanager --install "ndk;21.4.7075529"`（AGP 4.2 默认版本；sdkmanager 本身要 JDK 17+，用 `JAVA_HOME=<zulu-21> sdkmanager ...` 跑，别动项目的 JDK 8/11），装完删 `smart_message/build/intermediates/stripped_native_libs` 再重打。校验：正式包应为 **81.5MB（77.7MiB）**，比这大就是没 strip。阿里安全的 `libsg*.so` 报 `not recognized as a valid object file` 属正常（加壳文件，同事的包里也没 strip）。
