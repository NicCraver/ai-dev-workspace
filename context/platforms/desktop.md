...
- **dev 与 test 包会共用 userData（本地改名的副作用）**：`package.json` 的 `name` 若被本地改成 `zhixin-test`（提交禁忌里那个 `-test` 包名改动），就跟 `electron-builder.yml` 的 `productName: zhixin-test` 同名，`app.getName()` 一致 → dev 与装的包共用 `~/Library/Application Support/zhixin-test/`。上游原样是 dev=`zhiwulianxin`、打包=`zhixin-test`，本来分开。共用的不只是 `logs/`，还有 `sqlite3/`、`databases/`、`IndexedDB/`、`Local Storage/`、`electronStore/`、`Cookies`、`storage/`——两边同开会 LevelDB 抢锁、sqlite 并发写、登录态互顶。修法：本地把 `package.json#name` 改成不重合的值（如 `zhixin-dev-local`），`electron-builder.yml` 不用动（打包名只看 `productName`）。
  - **日志里区分两边看 `version` 字段**：`19.0.10`（= Electron 自身版本）是 dev——dev-runner 跑的 `dist/electron/main.js` 所在目录没有 package.json，`app.getVersion()` 退回 Electron 版本；`3.4.26-test` 这类才是打包产物（asar 内有 package.json）。
