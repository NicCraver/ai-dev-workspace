...
| 内联 HTML（`<span>` / `<mark>` + style） | 解析器原样吐出 raw HTML 节点，由既有 HTML 后处理二次加工。必须同时认 **字色与背景色**，且 `color:` 不能误匹配 `background-color`。`<mark>` 是合法 HTML 短语标签，不能当未知标签原样露出；无背景时用黄底。`padding` / `border-radius` 原生富文本做不到，矩形底色即可 |
