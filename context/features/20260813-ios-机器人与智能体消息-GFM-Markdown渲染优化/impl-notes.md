...
| 自己发到群里的个人 AI 框长卡片 | 发送人是本人、没有智能体前缀、知识来源常为空。折叠必须按「消息类型是 ActionCard 且超限高」，不能复用「是不是 AI 卡片」判据；后者只控制角标、知识来源、是否拼「回复 @」前缀。两端混用会：要么不折叠，要么误加回复前缀 |
...
- 2026-08-25：长卡片折叠与「AI 卡片渲染」是两件事。安卓对所有 ActionCard 超限高都折；若把折叠绑在「发送人带智能体前缀或知识来源非空」上，个人 AI 框自己发到群里的长回复（无前缀、知识来源空）就会整卡撑开且没有查看更多。折叠开关应对齐安卓：ActionCard 超高即折。AI 卡片判据只控制角标、知识来源、是否拼回复前缀。
