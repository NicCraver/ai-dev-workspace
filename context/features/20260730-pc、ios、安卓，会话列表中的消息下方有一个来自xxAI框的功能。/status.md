...
> 最后更新：2026-07-30（安卓/iOS 合并详情列表 badge 对齐 PC `126d78d6`）｜图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
| 合并转发保留字段 + 合并详情列表 badge | — | 🚧 对齐 PC（本地未 commit） | 🚧 对齐 PC（本地未 commit） | ✅ `126d78d6` |
...
| android | `cc79be12c`（右对齐 + pill 顺序）· `02cef0fdc`（来源）· `fe6d1687f`（详情）· `6ecb5ae92`（定时图标）；**本地未 commit**：合并保留字段 + **CombineAdapter 列表 badge** + cleanExtra 不抹掉 |
| ios | `5f3167a13`（来源）· `80d7870db`（详情+图标）· `1193d3595`（反序）；**本地未 commit**：合并保留字段 + **读 baseExtra/extra 兜底** |
...
- (android) ⏳ 真机：会话 badge + 合并详情列表 badge；逐条发出后无。本地待 commit / push。
- (ios) ⏳ 真机：同上。本地待 commit / push。
