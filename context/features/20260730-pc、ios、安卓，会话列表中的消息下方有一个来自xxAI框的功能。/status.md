...
> 最后更新：2026-07-30（安卓详情 badge 已 push `fe6d1687f`）｜图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
| 自己消息详情 badge | — | ✅ 仅循环时间（`fe6d1687f`） | 🚧 仅循环时间，未 commit | ✅ 触发名+循环时间（`ff400773`） |
...
| android | `02cef0fdc`（来源）· `fe6d1687f`（详情 badge 仅循环时间） |
| ios | `5f3167a13`（来源）；详情 badge **未 commit** |

## 待办 / 阻塞

- (旁路 · web · 选择AI框) ⏳ 群头像拼图 + 默认全部 tab + **停用 batchGetAgent**（只展示有 agentId）；代码已写未 commit（见 `20260707-选择AI框`）。
...
- (android / ios) ⏳ 真机：来源 badge + 自己消息详情双 pill（**仅循环时间**，无任务名）；他人/群仅来源。
- (ios) ⏳ commit + push `personal-ai-chat`（详情 badge）。
...
- 2026-07-30 安卓 / iOS：详情 pill 左侧加定时图标 16px，与文案间距 8px（`ic_personal_ai_schedule` / `icon_personal_ai_schedule`）
