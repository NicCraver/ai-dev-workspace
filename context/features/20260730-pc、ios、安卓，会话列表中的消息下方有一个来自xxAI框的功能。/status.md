...
> 最后更新：2026-08-06（安卓合并详情引用头+点引用弹窗已装真机；iOS 导航/回复层待真机）｜图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
| 转发保留引用 / 剥 referMsgUid（对齐 PC） | — | 🚧 逐条+合并页引用头/点引用弹窗已装真机（待重复合并自测+commit） | 🚧 保留 Reply + 剥 uid + 合并预览本地已改（待 commit/真机） | 🚧 本地已修（待 commit/真机） |
| 个人/群 AI 列表昵称取实时资料 | — | ⬜ | ✅ | ✅ |
| impl-notes | ✅ | ✅ | ✅ | ✅ |
| 自测通过 | — | ⬜ | ⬜ | ⬜ |
...
- (ios) ⏳ 合并详情导航左右图标 iOS26 居中；「N条回复」层加大顶距（本地已改，待真机）
...
- 2026-08-06：(ios) 合并详情 UI：导航 customView 在 iOS26 勿 Left/Right 对齐（圆形底图标偏）；`kPopoverNavTop = kNavBarH+20`，聚合层按 host 高度算顶距
