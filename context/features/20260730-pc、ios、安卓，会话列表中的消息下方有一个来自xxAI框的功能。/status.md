...
> 最后更新：2026-07-30（合并转发保留 badge 字段；逐条仍抹掉）｜图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
| 自己消息详情 badge | — | ✅ 仅循环时间 + 反序对齐 PC | ✅ 仅循环时间 + 反序对齐 PC | ✅ 触发名+循环时间（`ff400773`）+ 样式（`0b648606`） |
| 合并转发保留 badge 字段 / 逐条抹掉 | — | 🚧 本地未 commit | 🚧 本地未 commit | — |
...
| 端 | 提交 |
|----|------|
| desktop | `0b648606`（样式：定时图标 + 自己消息反序 + 间距）· `ff400773`（详情 badge）· `8444e7c3`（间距）· `2cb02be3`（功能）；旁路回执门闩已在 `a9bb3136` |
| android | `02cef0fdc`（来源）· `fe6d1687f`（详情）· `6ecb5ae92`（定时图标）；**本地未 commit**：pill 反序 + 间距 8dp + **合并转发保留 badge 字段** |
...
- (desktop) ⏳ 手测 badge：个人/群 × 定时/非定时；`"1"` 不显示；tag 不变；自己消息右侧详情 badge（触发名+循环时间；反序：详情内侧、来源贴右）。
- (android) ⏳ 真机：来源/详情 badge + 合并转发打开聊天记录仍有 badge；逐条转发无 badge。本地改动待 commit / push。
- (ios) ⏳ 真机：同上（合并有 / 逐条无）。本地改动待 commit / push。
...
- 2026-07-30 **转发**：逐条（含单条长按）继续抹掉 badge 字段（仅留 richList）；**合并转发** OSS txt 内保留 `fixTaskMessage` / `personalAccountId` / `dealForExtraInfo`（+ richList）；本期只改安卓/iOS
