...
> 状态：**已实现（desktop / android / ios 已 push）；待真机自测**

## 需求摘要（产品口径）

1. **群 AI 框**定时发到群内的消息：气泡**底部**增加标识「来自群AI框」。
2. **群 AI 框**与**个人 AI 框**定时发到群内的消息：均显示底部来源标识（个人为「来自{归属人昵称}个人AI框」，群为「来自群AI框」）。
3. 是否定时任务以消息字段 `content.extra.fixTaskMessage` 为准：`1` = 是，`0` = 不是；**仅数字 `1` 时显示**上述标识。
4. 昵称旁「个人AI框 / 群AI框」tag **不改**；只改气泡下来源 badge。

## 背景与目标

群聊中 AI 定时发出的消息需要在气泡下标出来源，便于区分普通 AI 回复与定时任务投递。非定时任务不显示该 badge（个人 AI 原先「有 personalAccountId 即显 badge」改为受门闩约束）。
