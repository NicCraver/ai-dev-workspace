...
> 最后更新：2026-08-12（stop-hook 收尾：矩阵仍全 ✅；apps 脏工作区均为旁路——android 正式包资源补丁、ios 合并引用快照 11 文件未提交、desktop 仅本地调试配置）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
- (android) 旁路：**回复弹窗**（点引用弹出的「N条回复」层）补 AI 框 tag + 评优徽章星星（`ReferMessageUnitFragmentDialog` / `ReferMessageUnitListAdapter` + `rc_item_refer_unit_*.xml`），并对空时间戳隐藏时间胶囊；合并打包补引用源 `referMsgSentTime`/内嵌 extra。**已提交 `f34ef9502`（未 push、AI 未编译）** → 详见 `20260730`
...
- (ios) 旁路：合并详情引用快照全链路修复（**11 文件，+417/-11 行**）：打包侧 `zx_packReferMsgDictionaryForModel` 写 `referMsg.user/sentTime/extra/objectName`；读侧 `hydrateCombineReferModelsInList` 同列表回填；`ZXIMCellLogic`/`ZXRCMessageModel`/`UIImageView+Avatar` 等联动。第四轮再补：快照带上**引用正文**（否则群智能体回复合并后整块引用消失）、extra 白名单保 `fromType`、读侧按快照直接建 `referModel`、`sentTime<=0` 隐藏时间（原显示「元旦 08:00」）。第五轮：合并详情昵称显示成融云占位串 `user<>`（`RCUserInfo` 的 description 格式），新增 `zx_isPlaceholderDisplayName:` 过滤（读侧三处取名 + 打包侧），并补「{拥有者昵称}个人AI框」兜底名。**工作区未提交、未构建**，待 Xcode 编译 + 真机自测，**老记录须重新合并**；待确认该记录由哪端合并 → 详见 `20260730`
...
- 2026-08-12：stop-hook 触发收尾；android 仍仅 `base_color.xml` + `em_camera_switch_normal.9.png` 资源补丁未提交；ios 合并引用快照 11 文件未提交；desktop 仍仅本地调试四文件；web 干净；矩阵不变；impl-notes 不变
