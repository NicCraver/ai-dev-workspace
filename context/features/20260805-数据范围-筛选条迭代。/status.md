...
- (ios) 旁路：合并详情引用快照全链路修复（约 11 文件）：打包侧 `zx_packReferMsgDictionaryForModel` 写 `referMsg.user/sentTime/extra/objectName`；读侧 `hydrateCombineReferModelsInList` 同列表回填；`ZXIMCellLogic`/`ZXRCMessageModel`/`UIImageView+Avatar` 等联动。第四轮再补：快照带上**引用正文**（否则群智能体回复合并后整块引用消失）、extra 白名单保 `fromType`、读侧按快照直接建 `referModel`、`sentTime<=0` 隐藏时间（原显示「元旦 08:00」）。第五轮：合并详情昵称显示成融云占位串 `user<>`（`RCUserInfo` 的 description 格式），新增 `zx_isPlaceholderDisplayName:` 过滤（读侧三处取名 + 打包侧），并补「{拥有者昵称}个人AI框」兜底名。**工作区未提交、未构建**，待 Xcode 编译 + 真机自测，**老记录须重新合并**；待确认该记录由哪端合并 → 详见 `20260730`
