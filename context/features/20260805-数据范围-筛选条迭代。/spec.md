...
2. 数据 `groupAndAccountSelectAll===1` 时外示「全部数据」；否则「数据+min(n,9)」；弹层内仍为真实数量。
...
1. 胶囊文案：
   ...
   - 否则（含字段缺省 / `undefined` / `0`，**缺省当 0**）→「数据+min(n, 9)」，n = `dataRangeScopeList` 长度（`null` 当 `[]`）
2. 用户点击「数据」胶囊：
...
3. 弹层内列表与勾选计数显示**真实数字**，不做 9 封顶，不用「全部数据」替代内部计数。
...
- 不把「全部数据 / 数据+9」文案规则套用到设置页、定时任务

## 验收要点

1. 个人 + 群：类型全选 →「全部类型」；取消一项 →「类型+x」
2. 数据 selectAll=1 →「全部数据」；=0 且 n>9 →「数据+9」；弹层真实数
