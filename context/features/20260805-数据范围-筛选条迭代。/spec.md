...
> 由 Superpowers brainstorm 产出。最后更新：2026-08-06
...
3. 点击「数据」胶囊：先 `getAgentDataRange`，成功后本地 scopes/flags 与胶囊对齐，再开层；失败仍用本地开层。
...
2. 用户点击「数据」胶囊：
   - **先**调用 `getAgentDataRange` 刷新记忆
   - 成功：回写父级 `dataRangeScopeList` 与三标记（有则更新；无标记则展示按缺省当 0），胶囊随之更新，**再打开**选人/群弹层（弹层编辑快照与胶囊同源）
   - 失败：可 `console.warn`；不强制 toast；本地记忆不回滚；**仍用本地数据开层**（避免卡住）
   - 进行中重复点击：忽略（由首次请求结束后开层）
...
- 聊天记忆筛选条：**数据**胶囊文案 + 点击时先 `getAgentDataRange` 再开层（个人 AI）
...
| 端 | 类型胶囊 | 数据胶囊 + 点击刷新 |
|----|----------|---------------------|
| web | `DataRangeBar`（FilterBar，含群） | `DataScopeBar`：文案；`openPicker` 先 get 再开层 |
| desktop | `personal-ai-memory-bar` + 群 `agent-memory-bar` | 个人 bar：先 get 回写再开 DataScope |
| ios | `ZXPersonalAiFilterBar` + 群 `ZXAIAgentFilterBar` | 个人：先 get 再 present 选人页 |
| android | 个人/群 FilterBar 对应文案处 | 个人 Host：`fetchAndBind(true)` 后再 open |

默认四端行为一致；无系统权限类差异。移动端原生选人页若开页仍会 get，属二次拉齐；父级胶囊须在返回/ACK 后按最新 flags 更新。
...
- get 失败：记忆不回滚、不强打断 toast；仍用本地数据开层
...
3. 点数据胶囊先 get；成功后胶囊/本地与回参一致再开层；失败仍开层（本地）
...
- 2026-08-05：点击数据先开弹层，并行 get（不阻塞打开）——**已废弃**
- 2026-08-06：改为先 get 回写再开层（避免弹层快照与胶囊不一致）；失败仍用本地开层
