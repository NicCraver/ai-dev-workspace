...
> 最后更新：2026-08-19（iOS 真机自测十一轮；⚠ 末两个提交的效果在工作区被回退，未提交，见「待办」）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
| T5f H5 文件预览去掉会闪的普通 HUD（视频分支保留） | — | — | ✅ | — |
| T5g 知识来源文件本地缓存命中 + 缓存目录按 agentId+docId 隔离 | — | — | ✅ | — |
| T5h 浮层展示时序（80ms 即出 + 最短停留 200ms） | — | — | 🚧 | — |
| T5i 提示文案统一为「正在打开...」 | — | — | ✅ 待编译 | — |
...
| ios | **`feat/ios-file-download-progress`** | ahead 20（基线 `origin/release`） | **脏 2** | **本功能** | `d48c63df8`；已 clean build + 真机自测多轮；未 push。⚠ 工作区 2 个文件把 `6398b2df1`+`d48c63df8` 的效果回退了，未提交 |
...
| H5 内点文件先闪一下普通 HUD | `ZXJSMediaAPI` 预览入口无条件弹 `ZXProgressHUD` | 文件分支去掉，视频分支（不走新浮层）保留（`753b2427d`） |
| 知识来源同一个文件每次都重下 | 该入口没有本地缓存命中判断 | 加 `isCachedFileUsable:expectedSize:`；缓存目录按 `agentId + docId` 隔离，避免同名文件互相覆盖（`edbf24258`） |
| 缓存命中仍慢 / 浮层来得晚 | 知识来源缓存命中同样卡在绿盾解密判定的联网往返；300ms 延迟对「稍慢一点」的路径显得没反馈 | 知识来源缓存命中也置 `notDecrypt`；延迟 300→150ms（`6398b2df1`） |
| 点了没反馈，但浮层又会闪 | 单一延迟值两头不讨好：调小就闪，调大就没反馈 | 延迟降到 **80ms**（人眼近乎无感）+ **最短停留 200ms**（露面后至少停这么久；用户主动取消则立刻关）（`d48c63df8`） |
| ⚠ 上面末两条在工作区被回退 | 未提交改动把延迟改回 300ms、删掉 `zx_runAfterMinShow` 与最短停留、去掉知识来源 `fromCache → notDecrypt` | **未确认是有意回退还是误操作**，见「待办」 |
...
- (ios) 工作区脏区里**混着两件事**，提交前分清楚：① 用户手动撤销的回退（延迟回 300ms、去掉最短停留与 `zx_runAfterMinShow`、知识来源不再跳解密判定）；② 本轮的文案统一（7 个文件）。要一起提交还是先 `git restore` 回退部分，由用户定
- (ios) 文案改动**未编译验证**（本仓库规定 AI 不跑 `xcodebuild`），需人工 clean build：新增 `extern NSString *const ZXFileLoadingTitle;` 在 `ZXFilePreviewLoadHUD.h`，`ZXRCIMChatLogic.m` 是**间接**引入该头文件的，重点看它能否编过

**下一轮自测重点**：

- (ios) **绿盾企业内的普通文件消息**：跳过解密判定后能否正常打开（这是本次唯一有业务风险的改动，见决策记录）
- (ios) 知识来源缓存目录按 `agentId + docId` 隔离后：不同智能体下的同名文件互不覆盖，同一文件二次点开走缓存
- (ios) 延迟展示的最终取值（80ms + 最短停留 200ms，或回退后的 300ms）：缓存命中应全程无提示；稍慢的应带着 5~10% 直接出现，不从 0 开始，且不闪
...
- (ios) 绿盾加密文件：下载段走到 ~97% 后继续到 99%，**全程只有一个浮层、文案不变（一直是「正在打开...」）**
- (ios) 智问（`ZXZhiWenController` / `ZXZhiWenSearchController`）预览文件：普通 HUD 文案也改成了「正在打开...」，只改字、行为未动
...
- 2026-08-19：浮层**延迟展示**（`ZXFileLoadingSessionShowDelay`），期间只在会话内记账；快路径全程不弹，避免一闪而过。取值演进 300ms → 150ms → **80ms + 最短停留 200ms**（`ZXFileLoadingSessionMinShowDuration`）：单一延迟值两头不讨好，改成「几乎立刻出 + 出来就别马上走」，取消时不受最短停留约束。⚠ 工作区当前把这条回退到了 300ms 无最短停留
- 2026-08-19：**提示文案全程统一为 `正在打开...`**（常量 `ZXFileLoadingTitle`，定义在 `ZXFilePreviewLoadHUD.h/.m`）。曾考虑按阶段切「正在下载 / 文件解密中」，最终否决：用户只关心「点了之后在打开」，中间走到哪一步是实现细节，切文案反而让人以为在做别的事。因此 `updatePollingProgress:` 里那段「进解密就改标题」已删（连带删掉只为它存在的 `polled` 字段）；`beginStageWithTitle:` 的 title 参数保留但各调用点都传同一常量。弃用 `加载中...`（本期被替换掉的旧 HUD 用语，易混）与 `请稍候...`（无信息量）。标点沿用既有三点 `...`，不混 `…`
- 2026-08-19：**非加密文件消息 + 本地缓存命中 → 跳过绿盾解密判定**（`params.notDecrypt = YES`），后扩展到知识来源缓存命中分支。原因：判定结果只缓存 24h，过期后重开本地已有的文件仍要联网往返，慢网下像卡死。**风险**：若绿盾企业内普通文件消息也需服务端解密，这条会漏解密——用户已确认采用，自测须专门验。⚠ 工作区当前把知识来源那半边回退了
- 2026-08-19：知识来源文件缓存目录按 **`agentId + docId`** 分目录，避免不同智能体下同名文件互相覆盖（`edbf24258`）
