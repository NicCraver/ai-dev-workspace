...
> 最后更新：2026-08-19（iOS 真机自测十一轮；⚠ 工作区已由人工回退到 ≈ `edbf24258`，且文案改到一半编不过，见「待办」）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
| ios | **`feat/ios-file-download-progress`** | ahead 20（基线 `origin/release`） | **脏 5** | **本功能** | HEAD `d48c63df8`；未 push。⚠ 工作区被人工回退到 ≈ `edbf24258`（撤掉 `6398b2df1`+`d48c63df8`），另有半截文案改动，**当前编不过** |
...
| ⚠ 上面末两条被人工撤销 | 用户手动回退，工作区 ≈ `edbf24258`：延迟回 300ms、删掉 `zx_runAfterMinShow` 与最短停留、去掉知识来源 `fromCache → notDecrypt` | 未提交；下一步是 commit 这个回退还是 `git restore` 待定，见「待办」 |
...
- (ios) ❌ **编不过**：`ZXAgentKnowledgeOpenLogic.m:424` 与 `ZXRCIMChatLogic.m:136` 用了标识符 `ZXFileLoadingTitle`，但全仓无定义 → `Use of undeclared identifier`。补法：`ZXFilePreviewLoadHUD.h` 里 `FOUNDATION_EXPORT NSString * const ZXFileLoadingTitle;`，`.m` 里赋 `@"正在打开..."`
- (ios) **文案只改了一半**：已换的是知识来源 / 聊天两处会话标题（改成常量）+ 智问两处 `ZXProgressHUD show:@"正在打开..."`（`ZXZhiWenController.m:92`、`ZXZhiWenSearchController.m:88`）。**未换**：`ZXMultiMediaClient.m` 540 / 688 / 743 三处 `@"文件加载中..."`、`ZXFilePreviewLoadHUD.m` 72 / 421 两处兜底文案。要不要一起统一待定
- (ios) **回退是人工有意为之**（用户已确认），工作区 ≈ `edbf24258`：**要么 commit 认下这个回退、要么 `git restore` 留住 `d48c63df8`**，别带脏区自测，否则不知道在测哪一版
...
- 2026-08-19：**提示文案**（候选已定，落地到一半）：入口统一 `正在打开...`，拿到真实字节换 `正在下载...`，解密段用既有 `文件解密中...`；会话 `startOrReuseWithTitle:` 换文案不重置进度。弃用 `加载中...`（本期被替换掉的旧 HUD 用语，易混）与 `请稍候...`（无信息量）。标点跟既有代码用三点 `...`，不混 `…`
