...
- (ios) **工作区那 2 个未提交文件是回退**：把 `ZXFilePreviewLoadHUD.m` 的延迟 80ms + 最短停留 200ms 改回 300ms、删了 `zx_runAfterMinShow`；把 `ZXAgentKnowledgeOpenLogic.m` 的 `openLocalFile(path, fromCache)` 改回单参、去掉缓存命中的 `notDecrypt`。**要么 commit（认下这个回退）、要么 `git restore`（留住 `d48c63df8`）**，别带着这个脏区继续自测，否则不知道在测哪一版
- (ios) **文案未落地**：本轮只出了候选（见决策记录），代码里仍是旧文案
...
- 2026-08-19：**提示文案**（本轮只定候选，未落地）：入口统一 `正在打开...`，拿到真实字节换 `正在下载...`，解密段用既有 `文件解密中...`；会话 `startOrReuseWithTitle:` 换文案不重置进度。弃用 `加载中...`（本期被替换掉的旧 HUD 用语，易混）与 `请稍候...`（无信息量）。标点跟既有代码用三点 `...`，不混 `…`
