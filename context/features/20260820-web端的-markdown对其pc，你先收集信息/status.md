...
- (ios) 2026-09-02 **机器人卡片 / 转发自 AI 框的表格比流式输出小一号（已改代码，请再验）**：两条路径共用同一个样式工厂，但喂进去的正文属性分流了——识别成智能体卡片才用 `FSC(kT)`（16，跟随大字号设置），否则写死 `Font(14)`。表格的格内字号、单元格内边距（`round(base*0.6)` = 10 vs 8）、列宽兜底全从这个 baseSize 派生，于是同一张表在卡片里比流式里小一号、颜色也更浅。转发到别的会话后发送人不带 `ga_`、知识来源列表也丢了，`isAgentCardMessage` 认不出来，必掉进 14 那支。已改成**字号 / 行距不再分流，只分流字色**（`Color_H1` / `Color_H2`），并同步改了预估行高那条路径（否则卡片底部会被裁）。**请对比看**：一条流式回复的表 + 一条转发来的卡片表，格子高度、字号、内边距应一致；再看普通（非 AI）机器人卡片，正文字号会从 14 变成 16，确认没撑破布局。
