...
> 最后更新：2026-09-03（**已收尾关闭**：四端改动全部在 origin，用户验收通过）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
| 一/两列表格 `width:100%` 铺满容器 | ✅ `dd0a47d` | ✅ `9a3b6d172` | ✅ `6e964addc` | ✅ `0d00470c` |
| 两列表首列内容 `min-width:5em`（标签列不被挤扁） | ✅ `dd0a47d` | ✅ 同左 | ✅ 同左 | ✅ 同左 |
| 三列以上单列上限 = 半个气泡（JS 写 CSS 变量） | ✅ `dd0a47d` | ✅ 同左 | ✅ 上限含 padding，同 commit | ✅ JS 打标，同 commit |
| 折叠超限一律裁到限高（不整块取舍） | — | ✅ 早已如此 | ✅ `8b0925fb8` | ✅ 早已如此 |
| 表格与前一块间距（前缀块不被压） | — | ✅ 16dp | ✅ `8b0925fb8` | — |
| 普通长文本消息折叠（非卡片） | — | ✅ 550dp | ✅ `b35488a17` | ✅ 已有 |
| 表格样式基准不按发送人分流（卡片 = 流式） | — | — | ✅ `9839d7393` | — |
...
> **2026-09-03 收尾**：下列所有「已改代码，请再验」条目均已由用户真机 / 客户端验收通过，功能从 ACTIVE 移除；
> 保留原文只作实现史。iOS 相关 commit（`6e964addc` `23fc027fc` `8b0925fb8` `b35488a17` `9839d7393`）
> 已确认在 `origin/master-3.5.32`；安卓在 `origin/master-3.6.23`；PC `0d00470c` 在 `origin/master-3.4.27`；
> web `dd0a47d` / `7294897` 随 `feat/data-scope-storage-group` 已 push。
>
> **真正未做、留待后续**：① iOS 卡片气泡内边距 16 vs 流式 12（差 8pt，≥3 列列宽上限差 4pt）——属气泡设计
> 差异，要不要统一等定；② 老 WebView 上 `:has()` / `cqw` 的降级只是「不铺满」，没做 MutationObserver + JS
> 兜底；③ PC `.env.test` / `electron-builder.yml` / `package.json` 仍是本地调试，禁止提交；
> ④ 旁路那条 `<reference>` 单独成行折行（属 `20260814-pc安卓-GFM-Markdown渲染对齐`）当时记的是「未 commit」，
> 现 desktop 工作区已干净，需另行确认它是被提交了还是丢了。

- (ios) 2026-09-02 **机器人卡片 / 转发自 AI 框的表格比流式输出小一号（已改代码，请再验）**：两条路径共用同一个样式工厂，但喂进去的正文属性分流了——识别成智能体卡片才用 `FSC(kT)`（16，跟随大字号设置），否则写死 `Font(14)`。表格的格内字号、单元格内边距（`round(base*0.6)` = 10 vs 8）、列宽兜底全从这个 baseSize 派生，于是同一张表在卡片里比流式里小一号、颜色也更浅。转发到别的会话后发送人不带 `ga_`、知识来源列表也丢了，`isAgentCardMessage` 认不出来，必掉进 14 那支。对比截图（智能体回复 vs 转发的 AI 框卡片）确认差的就是字号 + 字色两项，已改成**字号 / 行距 / 字色全部不分流**，卡片正文恒用 `agentMessageBodyTextAttributesWithColor:Color_H1`，并同步改了预估行高那条路径（否则卡片底部会被裁）。**请对比看**：一条流式回复的表 + 一条转发来的卡片表，格子高度、字号、内边距、文字深浅应一致；再看普通（非 AI）机器人卡片，正文会从 14pt 灰字变 16pt 黑字，确认没撑破布局。
- (ios) 2026-09-02 **未动**：卡片气泡内容可用宽 `kChatMsgContentW - 32`（内边距 16），流式气泡是 `- SS(24)`（内边距 12），差 8pt，≥3 列列宽上限跟着差 4pt。这属于气泡设计本身不同，要统一得改其中一端的内边距，等你定。
...
- (ios) 2026-09-02 **普通长文本消息不折叠（已改代码，请再验）**：群里那条「Markdown 渲染样式全量预览」是 `RC:TxtMsg`（定时任务推送，`fixTaskMessage=1`，非 ActionCard），iOS 的折叠只挂在 AI 卡片 / 流式回复上，普通文本气泡没有任何折叠，8000 多字整条铺开。安卓 `TextMessageItemProvider` 550dp 就折、PC 也折。iOS 已补：超过限高收起 + 底部渐变遮罩「查看更多」，展开后底部给「收起」。前两版真机整条白掉（连气泡都没有），两个根因叠一起：① iOS 正文控件垂直居中绘制，只夹控件高度会把绘制起点推到界外；② 气泡所在的容器没有高度约束，高度全靠「正文顶 → 正文底 → 气泡底」这条约束链撑，我把正文底部那一根去掉后气泡直接塌成 0。现在收起态 = 限制行数 + 控件高度取限高 + 保留底部约束链。**请看那条超长文本**：收起态高度约等于半屏多一点、正文从顶部开始显示、底部有查看更多；展开后能看全并能收起；再滚动几屏看普通短消息没被误夹。
