...
## ✅ 真机验收结果：5 项全过（2026-08-26）

| # | 项目 | 结果 | 决定性日志 |
|---|------|------|-----------|
| 1 | 群聊兜底 | ✅ | `群聊 TAE-4KA7 发于 08-24 15:01:06 \| 3/3 \| ...609=**水位**08-26 13:38:02  ...169=**水位**08-26 14:28:51`——3 人里 2 人由水位翻转，本地与服务端均无记录 |
| 2 | 群聊分母 | ✅ | 名单外的人表态后仍是 `0/1`，分子分母都没被改写 |
| 3 | 切账号不串 | ✅ | 切前 `会话 1880150187008081921_1_...169`，切后 `会话 1992862025348345858_1_...169 当前水位：(空)`，所有消息判未读。**终审 Important #2 实锤验证** |
| 4 | 引用缓存 | ✅ | `watch msgReceipt` 仅 9 行，无刷屏、无整条闪烁。Task 5b 生效 |
| 5 | 只增不减 | ✅ | 同群前后两次快照均 `14:30:54`，未变小。中间那次是另一个群（水位按会话隔离），非回退 |

顺带再次验到私聊兜底：`私聊 864-U5E8 发于 05-26 20:04:18 | 本地=- 服务端=- 水位=08-26 14:33:17 → **水位兜底**`
——5 月的消息，回执早已丢失，水位捞回来了。

### 验收方法本身的两条结论（已写进 impl-notes）

1. **「先确认未读再触发证据」构造不出来**——对方打开会话去表态时，会话打开本身就发了正常回执，
   本端在线就收到了，分不清是水位生效还是回执本来好使。**改为看日志的「来源」那一列。**
2. **不能用「翻历史表态」验证**——未加载的消息收集不到证据（功能上无损失，理由见 impl-notes 第三节）。
   必须对**当前屏幕上可见的消息**表态。

### 一个已知噪音（不影响上线）

去重键改成「证据身份」后，每次进入长会话会把已有证据全量报一遍（实测一个群 150+ 行）。
切会话时 `seenEvidence` 清空，来回切会重复刷。生产包里这些全被编译期消除。
嫌吵的话：把「未抬高」那行降级为只写 `window.__readWatermark.evidence`、不打 console 即可。

## ✅ 已修：一次事件触发 6 次全量重算（2026-08-26）
