...
| web | feat/data-range-week-work | 无 upstream | 脏(2)：`src/server/index.js` + `corpPlateAccountRel.js` | **本功能** | 疑似开始接树接口 mock，本回合未改代码 |
...
- (web) mock 按新契约 `weekWorkDataRangeTree` 构造四棵树；`src/server/module/corpPlateAccountRel.js` 已出现未提交草稿，接下时对齐契约
