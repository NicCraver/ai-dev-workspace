...
9. **发送载荷 `aiRoleId`**：群路径沿用现网值（会话 AI 框 roleId）；**个人路径固定 `"1"`**（对齐 PC/iOS）——群里可能根本没有 AI 框，此时现网值为空，个人 AI 会发出与 PC 不一致的载荷。
10. **联网胶囊只留图标、时间弹层右对齐**：与 iOS 收敛后的筛选条一致；时间弹层若按居中定位，条靠右时会被屏幕裁切。
