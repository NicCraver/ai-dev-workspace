...
3. 用户在「全部 / 组织群 / 外联群 / 组织架构」四个视图间切换勾选，或在搜索框输入做本地过滤后勾选。四处勾选共享同一个选中集合，实时互通。
...
- 「组织群」「外联群」各自独立 tab（不再合在一个「群聊」tab 里分两段），每个 tab 内「全选行 + 列表」
- 弹窗内搜索改为前端本地子串过滤
- 三个全选标记的派生计算与保存
...
| 导航结构 | 横向 tab：全部 / 组织群 / 外联群 / 组织架构 + 搜索框 | 纵向：顶部「选择联系人」「群聊」入口，下方「全部」+ 列表 | 同 android | 同 web PC |
...
**搜索**：全局搜（人 + 群），对 `targetName` 做子串匹配、忽略大小写，在内存全量数据上过滤，无网络请求；搜索结果可直接勾选，勾选写入同一 Set。
