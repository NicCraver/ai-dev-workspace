...
> 最后更新：2026-07-22（web：对话列表收起/展开改独立 SVG，不碰全局 side-*）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞 · — 本期不做
...
- (web) ~~**展开/收起对话列表图标**~~：`side-close.svg` ←「收起对话列表」；`side-open.svg` ←「展开对话列表」。FloatingDock / AgentList 等沿用原 name。**待** 视觉验收
