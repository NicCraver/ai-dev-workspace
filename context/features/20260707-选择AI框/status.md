...
> 最后更新：2026-07-24（ios 选择联系人/组织架构底栏统一 SS(56)）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞 · — 本期不做
...
> **本轮 apps 事实**（2026-07-24）：**android** 已 push `cf92a6493`——最近聊天/记忆返显过滤；工作区干净；**待** 真机 E2E。**ios** 工作区未提交——选择联系人/组织架构/搜索结果底栏统一 **`SS(56)`**（缓解双行贴边）；布局仍 `bottom=-safeH` + `BookBottomView` 描边；**待** 提交 + 真机视觉验收。**web** 干净。**desktop** 仅本地 test 打包，**勿提交**。矩阵 T8/T9/T10 仍 🚧。
...
- (ios) ~~**选择数据范围·组织架构底栏与选择联系人不一致**~~：高度统一 **`SS(56)`**（ContactCorp / LYGCompany / LYGCompanResult）；数据范围关 `bottomType`；布局 `bottom=-safeH` + `BookBottomView` 描边。**工作区未提交**；**待** 真机视觉验收（双行不贴边、顶线可见）
