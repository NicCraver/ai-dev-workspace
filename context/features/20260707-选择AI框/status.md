...
> 最后更新：2026-07-24（ios 底栏上边距 16 + 顶部分隔线，高度 SS(64)）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞 · — 本期不做
...
- (ios) ~~**选择数据范围·组织架构底栏与选择联系人不一致**~~：高度统一 **`SS(64)`**；`BookBottomView` 上边距 **16** + 顶部 `Color_Line` 0.5pt 分隔线；下边距 10；数据范围关 `bottomType`；布局 `bottom=-safeH`。**工作区未提交**；**待** 真机视觉验收
