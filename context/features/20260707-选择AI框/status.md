...
> 最后更新：2026-07-23（ios 组织架构底栏描边/安全区边距对齐选择联系人；android 最近聊天筛选已 push）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞 · — 本期不做
...
> **本轮 apps 事实**（2026-07-23 晚）：**android** 已 push `cf92a6493`——`DataRangeScopeHelper.acceptRecentConversation/acceptScope` + `SelectDataRangeActivity` 最近聊天/记忆返显过滤（对齐转发）；工作区干净；**待** 真机 E2E（最近聊天不再空名/裸 id）。**ios** 工作区未提交——组织架构/搜索 9 人上限放开 + `clearMaxCountLimit`；选择联系人入口 `ZXContactCorpController` 底栏无上限；组织架构 `LYGCompany`/`LYGCompanResult` 底栏与选择联系人 1:1；**待** 提交 + 真机 E2E。**web** 工作区干净（无功能 diff）。**desktop** 仅本地 test 打包，功能 tip `3dec824d`，**勿提交**。矩阵 T8/T9/T10 移动端与 desktop 仍 🚧。
...
- (ios) ~~**选择数据范围·组织架构底栏与选择联系人不一致**~~：高度 `SS(50)`；数据范围关 `bottomType`；底栏布局对齐 ContactCorp——内容条 `bottom=-safeH`（不铺满安全区）、`BookBottomView` 自带 `Color_Line` 描边（上下线+边距）。`LYGCompanResult` 同步描边。**工作区未提交**；**待** 真机视觉验收
