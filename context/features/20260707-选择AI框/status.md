...
> 最后更新：2026-07-24（ios 子页底栏对齐主页 SS(56)+1pt 顶线）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞 · — 本期不做
...
- (ios) **选择数据范围·子页底栏统一**：`ZXContactCorpController` / `LYGCompanyViewController` / `LYGCompanResultController` 在 `selectDataRangeMode` 下复用 `ZXSelectDataRangeBottomView`；高度对齐主页 **SS(56)**（联系人原 SS(60) 偏高已修）；顶线改 **1pt `#C9CFD9`**（原 SS(0.5)+Color_Line 在子页看不见）。黑底临时调试仍保留。**工作区未提交**；**待** 真机再对高度/顶线，验完改回白底
