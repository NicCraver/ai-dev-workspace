...
> 最后更新：2026-09-02（PC ActionCard 阴影改边框试效果）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞

2026-09-01 把所有自己发气泡调浅到 `#F0F5FF`。2026-09-02 按验收收窄：

1. markdown 标题 H1–H4 `#3E7EFF` 不动。
2. 普通自己发言退回线上色（PC `#CCE0FE`，安卓 / iOS `#DEE8FF`）。
3. 自己发的 ActionCard 保留卡体 `#F0F5FF` + 卡头 `#D7E5FF`。PC 正在试边框：去掉阴影，描边改成 `1px #E1E5EB`（`#F4F6F8` 叠在浅底上几乎看不见）。安卓 / iOS 仍是 `#F4F6F8`。
...
| 自己发 ActionCard 卡体阴影 | — | ⬜ | ⬜ | ❌ 已拿掉，改试 `1px #E1E5EB` 边框 |
| token 表登记 | ✅ | — | — | — |
...
| 端 | 分支 | 同步 | 脏区 | 活跃功能 | 备注 |
|---|---|---|---|---|---|
| desktop | master-3.4.27 | synced | 脏：卡体阴影改边框 + 本地调试勿提交 | **本功能** | 配色已 push `0d00470c` |
...
| 端 | 文件 | 改动 |
|---|---|---|
| PC | `chat-box.vue` 等四处 `.msg-box-actioncard` | 拿掉 `box-shadow`，边框改 `1px solid #E1E5EB` 试效果 |
...
- 三端真机 / 热更新：自己打的字 = 线上蓝；转发 AI 卡 / 定时用我身份 = 浅底 + 描边；**PC 现为 `1px #E1E5EB` 边框、无阴影**
- 2026-09-02 **PC 先加阴影，安卓 / iOS 未加** — 已改成试边框 `#E1E5EB`
...
- 2026-09-02 不做全端阴影。PC 试过 `0 2px 8px` 后改成 `1px #E1E5EB` 边框看效果（`#F4F6F8` 叠在 `#F0F5FF` 上几乎看不见）
