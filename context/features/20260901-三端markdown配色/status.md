...
> 最后更新：2026-09-02（安卓 / iOS 自己发 ActionCard 对齐 PC `0 0 4px` 阴影）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
| 自己发 ActionCard 卡体阴影 | — | 🚧 4dp ambient，待真机 | 🚧 CAShapeLayer 0/4/10%，待真机 | 🚧 `0 0 4px rgba(31,35,41,.1)` 待热更新 |
| token 表登记 | ✅ | — | — | — |
...
| ios | feat/ios-agent-date-range | synced | 脏：本功能阴影 + `ZXIMChatCell` 旁路 | **本功能** | `ZXGroupRobotCell` 四边内缩 4pt + shape 阴影 |
...
| iOS | `ZXGroupRobotCell.m` | shape 加 `shadowOffset=0 / radius=4 / 10% #1F2329`；气泡相对内容区四边内缩 4pt；cell 高度 +8pt；cell / contentView / messageContentView 不裁切 |
| context | impl-notes / status | 阴影从「仅 PC」改为三端同一 token |

## 待办 / 阻塞

- (android / ios) 真机验：复制/转发自己的 AI 卡 → 浅底 + 描边 + **四周均匀浅阴影**；普通自己发言仍是线上蓝、无阴影
