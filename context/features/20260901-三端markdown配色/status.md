...
> 最后更新：2026-09-03（安卓表格边框 `b9a928e54` 已 push；阴影仍待真机）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
- 安卓本功能已提交 `bc532ea3d` 并**按用户指定直推** `master-3.6.23`（该分支是联调主干，惯例走 MR，这次没走）。表格边框修补 `b9a928e54` 同分支已 push（表体白底后外壳左/上线被盖住，改由首行首列补）。PC 已按用户指定推到 `master-3.4.27`（`0ed5a52e`）
