...
> 最后更新：2026-09-03（**已收尾关闭**：PC `3570d8c5` 已进 `master-3.4.27` / `release`，iOS `9d389e3d7` 已 push，两端验收通过）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
> 2026-09-03 收尾：PC / iOS 运行时验收通过，功能从 ACTIVE 移除。下面几条是本次没做、留给后续的事项。

- ~~运行时自测未完成~~ 2026-09-03 用户验收通过（跑法备忘：`npm run dev:prod`，Node 24 会让 webpack4 报 `ERR_OSSL_EVP_UNSUPPORTED`，须用 `~/.vite-plus/js_runtime/node/14.21.3`）。
