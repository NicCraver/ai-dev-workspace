...
| 验收清单（可交互 HTML，31 条 / 12 条阻塞） | — | — | — | ✅ `acceptance.html`（`ea8676e`），结果存 localStorage、可导出 Markdown |
| **真机验证** | — | ⬜ | ⬜ | 🚧 **仅跑了 D0**，见下；其余 30 条未测 |

## 真机验收进展（2026-08-25）

清单：`acceptance.html`（浏览器打开，逐条点选，结果自动存本地，可导出 Markdown）。
纯文本版 `acceptance.md`（23 条，未含终审补的 7 条回归项；**以 HTML 版为准**）。

### D0 · 服务端 `chatType:2` 探测 —— 有输出，但结论未定

实测输出：

```
[receipt] 群已读服务端返回 条数= 0 含按人明细= false
```

**代码行为正确**：能打印这行说明请求成功（`code !== "M0000"` 会 `Promise.reject()` → 走 `.catch` → 不打 log），
服务端返回的是空数组，`hasPerUserDetail` 因此为 false。

**但不能据此下结论**，条数=0 有三种可能，尚未区分：

| 可能 | 含义 |
|---|---|
| 服务端对 `chatType:2` 不返回数据 | 权威源对群聊无效，第 3 层只覆盖私聊 |
| 服务端支持，但**该群**确实无已读记录 | 换个群就有 |
| 参数或权限问题 | 需看 Network 请求体 |

**区分办法**（下回合做）：

1. 换 `2044600546811285505`（AI框自定义Agent）—— 8/24 R7 在该群验过，@李峰那条已读确实翻转过、有真实数据
2. 拿私聊做对照（同一接口、只差 `chatType`）：私聊 `innerReadTime` 有数据而群聊恒为 0 → 基本坐实只支持私聊
3. Network 面板过滤 `getReadMessage`，核对请求体的 `chatType` / `targetId` / `accountId` 与响应 `code`

**不阻塞验收**：`hasPerUserDetail` 本就是为这个不确定性设的门闩——false 时群聊退化为
「仅本地回执」，与改动前完全一致、不会更差。

若最终确认服务端不支持群聊 → 记一条**后端待办**：需补群维度按人已读查询。
这是唯一能解决「换电脑群已读全丢」的路径。
