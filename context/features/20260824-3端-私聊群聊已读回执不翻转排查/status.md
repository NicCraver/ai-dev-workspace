...
| desktop | `fix/pc-read-receipt-hardening` | ahead origin/release 6（`d02faddd` + `e7393131` + `650e5792` + `0297ff61` + `63501144` + `0f4db746`） | 脏 3 | **本功能**：Task 1+2+3+4+4B+5 已提交。脏 3 仍是本地打包配置，**禁止提交**：`.env.test` / `electron-builder.yml` / `package.json` |

> 2026-08-24 PC 加固 Task 1+2+3+4+4B+5 已落地：`readStateModel.js` 8 个导出 + 单测 43/43；`receiptMetrics.js` 已被 `msg-list.vue` import（模块加载会挂 `window.__receiptMetrics`），但 Task 5 尚未 `bump`。Task 5 已把 `resolvePrivateReadTime` 接到 `getStatusText`，并新增 `electronStore/ensureReadTimeDates` 按需补载超窗口册子。GUI 未在本回合验证。表态/回复反推要等 Task 10 接线后才影响展示。web / 安卓 / iOS 脏区仍是同日旁路（Markdown 展示），不是 IM 回执。
