...
## 首屏快捷建议（2026-08-27 增补）

输入框 placeholder 改为「告诉我时间和人数，帮你找会议室」。

助手打开且尚无状态或结果卡片时，在输入框上方展示 4 个快捷建议：
...
时间均按上海时区计算并对齐 30 分钟格。若今天下午已经过去，则第三项顺延到明天下午。

服务端根据当前用户历史预订统计常用开始时段、时长和会议室，用可信度最高的历史偏好替换最不相关的通用建议；历史不足时全部使用通用时间规则。快捷建议只返回展示文案和完整自然语言请求，不直接返回可确认草稿。

前端通过轻量建议接口加载。点击建议后复用现有 `action: "message"` 流程，让助手查空房并进入既有的查询卡、选档和确认链路；不得直接创建预订。建议接口失败时静默隐藏快捷建议，手动输入仍可正常使用。

测试补充：

- 上海时区下一半点、下午过期顺延和跨日边界；
- 历史不足时返回 4 个通用建议；
- 有历史时按常用时段、时长和会议室替换建议；
- 点击建议发送对应 `message`，不直接写入 booking。
