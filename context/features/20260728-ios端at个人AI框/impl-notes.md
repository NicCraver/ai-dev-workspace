...
7. **个人 vs 群 `dataRangeList` 序列化形状不同**：群路径历史逻辑常固定补齐 type `0/1/2`；个人 get 实测为 `3/4/1/2`（无 0）。若发送/`aiRobtChat` 仍走「只吐 0..2」的序列化，会丢掉 3/4 并补出假的 type0——个人路径必须**原样透传**记忆条当前列表。
8. **`saveDataRange` 禁止空 `dataRangeList`**：记忆未回填或筛选条缓存被清空时若仍 save `[]`，会覆盖服务端记忆。应优先读筛选条现场值；列表为空则跳过 save（仅日志），等 get 回填后再改再存。
9. **筛选条 UI 收敛（个人）**：知识类型胶囊文案「类型+N」（不展示类型图标栈）；DataScope「数据+N」；时间文案不变；联网只保留图标、隐藏「联网」文字。时间弹层相对时间胶囊**右对齐**并可略右偏；宽度需容纳「近一年」等长文案（群侧可更宽）。
10. **个人 `@` 须写入 `extra.atUserList`**：组装发送扩展时若只认「群绑定智能体账号」或群成员，个人 AI（`ga_` / `agentKind=personal`）会被漏掉，气泡高亮依赖 `atUserList` 的起止下标，发出去后 `@` 不高亮。智能体（群/个人）均应按 `ga_` 或 `agentKind` 写入。
