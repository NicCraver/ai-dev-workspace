...
> 最后更新：2026-08-17 09:23（web 标题栏「涉密」按钮改黄色已 commit+push `749d1f3`）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
> 本功能代码侧：web `origin/feat/data-scope-secret-tag` `749d1f3`（标题栏按钮黄色）；android/ios/desktop 各自 `origin/personal-ai-chat-hotfix`（android `f5f2d0ce3`、ios `8b6dbf809`、desktop `763cd15e`）。四端 `getSecretButtonTip` 已 push。旁路脏区仍在：android 相机资源、ios combine、desktop `electron-builder.yml`/`package.json`。
...
- (web) 2026-08-17：标题栏「涉密」按钮原先图标黄、文字灰（`#5D616B`），已改成整颗按钮 `text-#FEAC00`（与行内 tag / `secret.svg` 同色）。已 push `749d1f3`，**未浏览器复验**
...
- 2026-08-13：四端 `getSecretButtonTip` 已 commit 并 push（web `e2640d6` → `feat/data-scope-secret-tag`；android `f5f2d0ce3` / ios `8b6dbf809` / desktop `763cd15e` → 各自 `personal-ai-chat-hotfix`）。矩阵「接口文案」/「自测」仍 🚧。旁路脏区未提交。
