...
## 7.1 ⚠️ 滚动容器不吃 `longClickable` 标志位，长按必须自己判（2026-08-27 真机打回）

上面第 115 行「`longClickable` 打回 true 才吞长按」这条**对滚动容器不成立**，当时以为顺带修掉了、其实没有：真机上按在表格区域仍然弹不出转发/回复菜单，按在同一条消息的文字区域正常。

**根因**：横滚容器（安卓 `HorizontalScrollView`）的触摸处理是**自成一套**的——它自己处理全部触摸事件、**从不回调基类的 `onTouchEvent`**，因此
1. 不看 `clickable` / `longClickable` 标志位，把它们设成 false 完全无效；
2. 基类里那套「按下计时、超时判长按」的机制根本没机会跑，容器自己也不会长按；
3. 只要它有子 View，按下事件一律被判为已消费，**不再向上冒泡**，挂在气泡根上的长按监听收不到手势。

段里的纯文本能长按，是因为文本控件在不可点击时按下事件返回「未消费」会冒泡；而表格段是整条通栏宽，等于在气泡中间挖了一条长按死区。

**做法**：横滚容器自己判长按再上抛。
- 按下起计时（用系统长按时长），超时未移出滑动阈值就沿 parent 链逐级 `performLongClick`，第一个消费的祖先（气泡根）弹菜单；
- 移动超阈值 / 抬起 / 收到 cancel 一律撤销计时——横滚表格、列表纵滚都不会误弹；
...
> 通用教训（三端同理）：**列表 item 里的任何滚动容器都会截断父级手势**，长按/点击这类要冒泡的交互，得由滚动容器显式转交，不能指望标志位。iOS 侧同一位置是 `UIScrollView` 的手势识别器，注意别重蹈。
