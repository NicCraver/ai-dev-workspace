...
> **验收基准是 Node 端已有的 77 个测试用例**（`server/tests/` 下 booking 23 / routes 16 / httpApi 10 / availability 9 / time 8 / room 6 / dict 4 / user 1，不含 agent 系列）。移植时逐条对照测试名打勾，别只照着源码读。
> 提示文案逐字照抄，前端直接展示 `msg`。

摘自 `server/src/domain/booking.ts` 与 `room.ts` / `dict.ts`，漏一条就有行为差异：
...
**默认主题**：未填主题时为「<姓名>预定的会议」；**无姓名时是「同事预定的会议」**；姓名过长先截断，保证总长不超 50。

**归属校验**：修改与释放前必须比对 `hostUserId`。**非本人一律返回 `M4004`「预定不存在」，不是 `M4003`**——避免泄漏他人预定的存在性。

**会议室校验（11 条，全部 `M4000`）**：名称非空且 ≤30 字 /「请输入名称」「名称不超过 30 个字」；分组 ≤20；位置备注 ≤100；楼宇非空「请选择或输入建筑」；楼层必须在 1层–20层 枚举内「请选择楼层」；容纳人数 1–999 整数；开放时间可解析「请选择开放时间」；结束必须晚于开始「结束时间必须晚于开始时间」；可提前范围只能 7/30/90/180「请选择可提前预定范围」；设施必须已知且启用「存在未知设施」（**编辑时允许保留原本已停用的设施**）；楼宇必须是启用中的「请选择启用中的建筑」（**编辑时允许保留原值**）。另有**启用态会议室不可重名**，停用后可重名，重新启用时再查一次。

**会议室默认值（照抄 Node，别自己定）**：`openStart=07:00`、`openEnd=23:00`、`bookAheadDays=90`、`needApproval/allowRecurring/allowPreempt=false`、`enabled=true`。设施入库前**去重并按设施字典的 sort 顺序排列**。

**字典规则**：名称 ≤20 字，同类型重名「同类型下已有相同名称」，`sort` 只接受正整数否则归 1，不存在时「字典项不存在」。**改名必须级联**：楼宇改名 → 重写所有会议室的 `buildingName`；设施改名 → 逐个重写 `rooms.facilities` 里的该项。漏了会让会议室指向不存在的字典项。
