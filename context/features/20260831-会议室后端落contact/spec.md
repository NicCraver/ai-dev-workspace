...
- **一期只对美腾（`corpId=6`）开放**。加配置项 `meeting.corp.whitelist=6`（逗号分隔），不在白名单的企业访问任何会议室接口一律拒，返回 `M4005`「当前企业未开通会议室」。
  - `6` = 天津美腾科技股份有限公司（简称「美腾」），2026-08-31 在测试库 `zx_contact` 查证。PC 端 `store/getters.js:30` 也把 `corpId === "6"` 当默认企业硬编码。
  - ⚠️ **`6` 只在测试库验证过，生产库未验证**。所以走配置项，**不许硬编码进代码**。项目已接 Nacos 配置中心，白名单可做成动态刷新，扩企业时不必重启。
  - ⚠️ 库里还有「美腾科技」`156695132969185`、「天津美腾资产管理」`1334685927934722050`、「海纳美腾」`1897853433856151554` 等同名系企业，一期都不放开，别按名字匹配。
  - `M4005` 是本功能新增的码（Node 端没有）。前端对它无分支依赖，只弹 `msg`，安全。
- **白名单之外的表数据照常按 corpId 存**，将来放开企业只改配置，不动表结构、不迁数据。
