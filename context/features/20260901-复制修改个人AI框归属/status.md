...
> 最后更新：2026-09-03（**已从 ACTIVE 移除**，按用户指示不再跟进；三端代码已 push，plan 不再补）｜ 图例：⬜ 未开始 · 🚧 进行中 · ✅ 完成 · ❌ 阻塞
...
2026-09-03 按用户指示从 ACTIVE 删除，不再跟进。已 push 的代码保留（web —、安卓 / iOS 已验、PC `e2e8b40b`）。
遗留但不再排期：PC 客户端那一遍目视（复制自己的 `@个人AI框` → 蓝色 + 个人筛选条「全部类型」；复制别人的 → 黑字无条）、
plan 不再补。目录留档，需要时重新加回 ACTIVE。
