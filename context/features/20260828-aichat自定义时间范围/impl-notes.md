...
### 5. iOS：选项行「请选择时间」但 webview 里已有选中日期

选项行在**当前档不是自定义**时框内恒走占位，但点自定义仍把条上缓存的
`customStartTime/customEndTime` 拼进 URL。切过自定义再切回近一周、或换会话没 reset
干净时，占位与日历回填就会打架。

判据与安卓同一份：`timeType == 0 && start > 0 && end > 0` 才带 query；否则只开
`?platform=m`。切到固定档 / 回填非自定义档 / reset 时把本地区间清 0。
