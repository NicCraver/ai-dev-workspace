...
2. 原生拉起 web 的 `/date-range` 免鉴权独立页（iOS/安卓半屏 webview，PC iframe），
   URL 带 `?platform=m|pc`；**仅当前档已经是自定义且两端有效**才再带 `startTime/endTime` 做返显。
   选项行显「请选择时间」（当前不是自定义、或没有区间）时 URL **不得**带区间——
   否则 H5 会按 query 高亮一段日期。切回固定档时要把本地旧区间清掉（对齐安卓 DTO 置 null）。
...
**区间文案的年份口径**——⚠️ 移动端（ios/android）与 web/PC **有意不同**，别互相"拉齐"：

| 区间 | ios / android | web / PC |
|------|---------------|----------|
| 起止都在当年 | `05/06~08/31` | `5/6~8/31` |
| 跨年（起 2025 止 2026，今年 2026） | `25/12/10~26/06/26` | `25/12/10~6/26` |
| 同年但非当年 | `24/05/06~24/08/31` | `24/5/6~24/8/31` |

- **ios / android**（2026-08-31 定）：起止**都在当年**才两端都不带年；只要有一端不在当年，
  **两端都带**两位年。月/日一律补零两位（`8/9` → `08/09`）。
- **web / PC**：每端各自判是否本年（本年 `M/D`，非本年 `YY/M/D`），月/日不补零。本轮没动这两端。

被否掉的写法：「同年只在左端带一次年」——起止同为去年时右端漏年，`24/5/6~8/31`
会被读成今年（iOS 原写法，已修）。
...
`ZXAIAgentTimeRangeBoxLabel`（声明在 `ZXAIAgentFilterBar.h`）。均无单测。
...
### 5. 选项行「请选择时间」但 webview 里已有选中日期

选项行框内文案才是用户看到的状态：当前档不是自定义、或自定义但没有区间文案时，
框里都是占位「请选择时间」。点自定义却把条上缓存的旧区间拼进 `/date-range` URL，
H5 会按 query 高亮一段日期——占位与日历打架。

回填必须三件事同时成立，缺一就把 URL 收成只带 `?platform=m`：

1. 当前档已经是自定义
2. 本地起止毫秒都有效
3. 选项行框里正在显示区间文案（不是占位）——避免「档是自定义、框仍是占位」时把缓存区间带进去

切到固定档、get 回填不是「自定义+有效区间」、reset 时，都要把本地区间清掉。
get 回填若是自定义但两端无效，也要清区间、保持占位，不能留下上一会话的毫秒。

### 6. web：筛选条已是「自定义」，打开列表却高亮「近一周」
