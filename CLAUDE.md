...
| 路径 | 说明 |
|------|------|
| `apps/meeting/` | 智能会议室。**目前是个人项目**（远端在 GitHub `NicCraver/zx-meeting-room`，其余仓库都在公司内网 GitLab），正准备推进到公司。前端 Vue；后端在 `apps/contact`。内嵌于智信三端 WebView。约定见 `context/platforms/meeting.md` |
